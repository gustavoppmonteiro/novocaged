...
baixa_caged <- function (ano, mes_inicial, mes_final = mes_inicial) {
      `%>%` <- magrittr::`%>%`
      meses <- sprintf("%02d", c(mes_inicial:mes_final))
      for (i in seq_along(meses)) {
            origem = paste0("ftp://ftp.mtps.gov.br/pdet/microdados/NOVO CAGED/", 
                            ano, "/", ano, meses[i], "/CAGEDEXC", 
                            ano, meses[i], ".7z")
            destino = paste0("CAGEDEXC", ano, meses[i], ".7z")
            
            
            result <- tryCatch(
                  {
                        curl::curl_download(url = origem, 
                                            destfile = destino, 
                                            mode = "wb")
                  },
                  error = function(e) {
                        utils::download.file(url = origem, destfile = destino, 
                                             mode = "wb")
                        NULL
                  }
            )
            ...
            origem = paste0("ftp://ftp.mtps.gov.br/pdet/microdados/NOVO CAGED/", 
                            ano, "/", ano, meses[i], "/CAGEDFOR", 
                            ano, meses[i], ".7z")
            destino = paste0("CAGEDFOR", ano, meses[i], ".7z")
            result <- tryCatch(
                  {
                        curl::curl_download(url = origem, 
                                            destfile = destino, 
            ...
            origem = paste0("ftp://ftp.mtps.gov.br/pdet/microdados/NOVO CAGED/", 
                            ano, "/", ano, meses[i], "/CAGEDMOV", 
                            ano, meses[i], ".7z")
            destino = paste0("CAGEDMOV", ano, meses[i], ".7z")
            result <- tryCatch(
                  {
                        curl::curl_download(url = origem, 
                                            destfile = destino, 
